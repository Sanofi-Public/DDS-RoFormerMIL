...
class WholeSlideImage(object):
    ...
    def segmentTissue(
        ...
        if use_simple_seg:
            img_otsu = simple_segmentation(self.path, mag_thumb=seg_level).astype(
                int
            )  # we want 1/0 instead of true/false
    ...
    def process_contour(
        ...
        scaled_coords = (scale_factor * results).astype(int)
        print("Extracted {} coordinates".format(len(results)))

        if len(results) > 1:
            asset_dict = {"coords": results, "scaled_coords": scaled_coords}
            ...
            attr_dict = {"coords": attr, "scaled_coords": scaled_attrs}
