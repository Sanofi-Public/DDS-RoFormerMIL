...
class Generic_WSI_Classification_Dataset(Dataset):
    ...
    def summarize(self) -> None:
        log.info("label column: %s", self.label_col)
        log.info("label dictionary: %s", self.label_dict)
        log.info("number of classes: %s", self.num_classes)

        for i in range(self.num_classes):
            log.info(
                "Patient-LVL; Number of samples registered in class %d: %d",
                i,
                self.patient_cls_ids[i].shape[0],
            )

            log.info(
                "Slide-LVL; Number of samples registered in class %d: %d",
                i,
                self.slide_cls_ids[i].shape[0],
            )
...
class Generic_MIL_Dataset(Generic_WSI_Classification_Dataset):
    def __init__(self, data_dir, h5_folder: str = "h5_files_grid", **kwargs):
        super(Generic_MIL_Dataset, self).__init__(**kwargs, h5_folder=h5_folder)
    ...
    def __getitem__(self, idx) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if not self.use_h5:
            full_path = os.path.join(
                self.data_dir, "pt_files", "{}.pt".format(slide_id)
            )
            features = torch.load(full_path)
            coords = torch.tensor([])

        else:
            full_path = Path(self.data_dir) / self.h5_folder / f"{slide_id}.h5"
...
class Generic_Split(Generic_MIL_Dataset):
    def __init__(self, slide_data, data_dir=None, num_classes=2, h5_folder="h5_files"):
        self.use_h5 = True
        self.slide_data = slide_data
        self.data_dir = data_dir
