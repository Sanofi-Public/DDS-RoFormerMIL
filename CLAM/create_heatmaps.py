...
from vis_utils.heatmap_utils import compute_from_patches, drawHeatmap, initialize_wsi
from wsi_core.batch_process_utils import initialize_df
from wsi_core.wsi_utils import sample_rois

from romil.eval_utils import initiate_model as initiate_model
...
if __name__ == "__main__":
    ...
    if data_args.process_list is None:
        if isinstance(data_args.data_dir, list):
            ...
            for data_dir in data_args.data_dir:
                slides.extend(Path(data_dir).iterdir())
        else:
            slides = sorted(Path(data_args.data_dir).iterdir())
    ...
    Path(exp_args.production_save_dir).mkdir(parents=True, exist_ok=True)
    Path(exp_args.raw_save_dir).mkdir(parents=True, exist_ok=True)
    blocky_wsi_kwargs = {
        "top_left": None,
        "bot_right": None,
    ...
    for i in range(len(process_stack)):
        ...
        Path(p_slide_save_dir).mkdir(parents=True, exist_ok=True)

        r_slide_save_dir = os.path.join(
            exp_args.raw_save_dir, exp_args.save_exp_code, str(grouping), slide_id
        )
        Path(r_slide_save_dir).mkdir(parents=True, exist_ok=True)
        ...
        Path("heatmaps/results/").mkdir(parents=True, exist_ok=True)
