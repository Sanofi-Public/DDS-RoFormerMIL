...
def seg_and_patch(
    ...
    slides = sorted(list(Path(source).iterdir()))
    ...
    for i in range(total):
        ...
        if auto_skip and os.path.isfile(os.path.join(patch_save_dir, slide_id + ".h5")):
            ...
            lst_slides.append({"slide_id": slide_id})
            dfslides = pd.DataFrame(lst_slides)
            dfslides.to_csv(os.path.join(save_dir, "slides.csv"), index=False)
            continue

        # Inialize WSI
        full_path = os.path.join(source, slide)
        WSI_object = WholeSlideImage(full_path)
        # if not mag info, skip it
        if "openslide.objective-power" not in WSI_object.wsi.properties.keys():
            continue

        mag = int(WSI_object.wsi.properties["openslide.objective-power"])
        factor = int(mag / desired_mag)
        # if hr image is lower res than desired skip
        if factor < 1:
            continue
        patch_size_current = factor * patch_size
        step_size_current = factor * step_size
        target_size = -1 if factor == 1 else patch_size
...
parser.add_argument("--desired_mag", type=int, default=20, help="desired magnification")
parser.add_argument("--patch", default=False, action="store_true")
parser.add_argument("--seg", default=False, action="store_true")
parser.add_argument(
    "--use_simple_seg",
    default=False,
    help="use the simple segmentation instead of CLAM segmentation",
)
...
if __name__ == "__main__":
    ...
    seg_params = {
        ...
        "keep_ids": "none",
        "exclude_ids": "none",
        "use_simple_seg": args.use_simple_seg,
    }
    filter_params = {"a_t": 100, "a_h": 16, "max_n_holes": 8}
